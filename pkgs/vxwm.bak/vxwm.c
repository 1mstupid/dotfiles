...
/* rebuild occ/urg safely (keep your original logic if already correct) */
for (c = m->clients; c; c = c->next) {
    /* IMPORTANT:
       if c->tags is bitmask (dwm style) -> use: occ |= c->tags;
       if c->tags is index style -> use: occ |= (1 << c->tags);
    */
    occ |= c->tags;
